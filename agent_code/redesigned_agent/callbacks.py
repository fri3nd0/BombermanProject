...
def state_to_features(game_state, r=4, last_action=None):
    """
    *This is not a required function, but an idea to structure your code.*
    ...
    return channels[:, x_l:x_u, y_l:y_u], coin_map, features, act_map
...
def act(self, game_state: dict) -> str:
    ...
    channels, coin_map, features, act_map = state_to_features(game_state)
    ...
    logits = self.model(torch.tensor(channels[np.newaxis,np.newaxis], dtype=torch.float, requires_grad=False),
                        torch.tensor(coin_map[np.newaxis, np.newaxis], dtype=torch.float, requires_grad=False),
